...
if st.session_state.page == 'home':
    ...
    st.markdown("<hr style='border:none; border-bottom: 2px dashed #000; margin: 10px 0 30px 0;'>", unsafe_allow_html=True)
    ...
    with ctrl_col1:
        ...
        m_choice = st.selectbox("Detection Model", ["yolov8n.pt (FAST MODE)", "yolov8s.pt (ACCURACY MODE)"], index=0 if 'n.pt' in st.session_state.model_name else 1, label_visibility="collapsed")
    ...
    with ctrl_col4:
        ...
        uploaded = st.file_uploader("Upload MP4", type=['mp4'], label_visibility="collapsed")
        if uploaded:
    ...
    with col_h1:
        st.markdown('<div class="hero-title" style="font-size: 2.1rem; letter-spacing: -1px;">Live Monitoring Phase</div>', unsafe_allow_html=True)
